...
export default {
  ...
  methods: {
    async handleSubmit() {
          if (!this.newMessage.content.trim()) return; // evita mensajes vacíos

          try {
            await sendGlobalChatMessage({
              sender_id: this.user.id,
              email: this.user.email,
              display_name: this.user.display_name, // se guarda con nombre visible
            ...
            // Hacer scroll al final (asegura ver el último mensaje)
            await this.$nextTick();
            ...
            <ol class="flex flex-col gap-4">
               <li
                  ...
                  <div class="mb-1 text-sm">
                     <span
                        :class="message.email === user.email ? 'font-bold text-gray-100' : 'font-bold text-yellow-500'"
                        >
                  ...
                  <div 
                     :class="message.email === user.email ? 'text-xs text-gray-800' : 'text-xs text-gray-400'"
                     >
